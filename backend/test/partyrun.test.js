...
test('publicRun never exposes an unrevealed (hidden) enemy', () => {
  for (let s = 1; s <= 30; s++) {
    const run = pr.createPartyRun(humans(), seededRoller(s)); rollInit(run, seededRoller(s + 1000));
    const v = pr.publicRun(run);
...
test('clearing a room heals the party and lets a member descend', () => {
  ...
  assert.strictEqual(cleared.run.phase, 'initiative', 'the GM calls for initiative');
  assert.ok(pr.applyAction(cleared.run, 'c1', { type: 'initiative' }, cleared.roll).ok);
