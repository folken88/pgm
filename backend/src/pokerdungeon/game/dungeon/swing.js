...
module.exports = {
  ...
  _swingVsAC(attacker, ac, target, extraToHit = 0, offHand = false) {
    ...
    const sneakDice = sneakOk ? Math.min(SNEAK_DICE_CAP, Math.max(1, Math.ceil(lvl / 2))) : ((cls === 'investigator' && studied && lvl >= 4) ? Math.min(SNEAK_DICE_CAP, Math.floor((lvl - 2) / 2)) : 0);   // investigator STUDIED STRIKE (PF1 ACG, v1.20.16): 1d6 at L4, +1d6 per 2 levels vs the STUDIED foe
    ...
    // PF1 RAW (poker v3.37.95 hand-port): POWER ATTACK is MELEE-only, DEADLY AIM is
    // RANGED-only — both stances fold into m.buffs, so back the wrong-kind stance
    // out of THIS swing (the improvised crossbow was shooting with PA's −4/+12).
    ...
    const toHit = bab + _ap.toHitMod + (weapon.toHit || 0) + arcEnhDelta + smiteHit + baneHit + (buff.toHit || 0) + _stanceHit + pbs + flankHit + studiedN + extraToHit + notProf - sick - (attacker.grappled ? 2 : 0) - (attacker.slowed > 0 ? 1 : 0) - (attacker.prone && !(weapon && weapon.ranged) ? 4 : 0) + _dStrike + ff.hit + swashWF;   // PF1: a prone attacker takes −4 on MELEE attacks (ranged unaffected here — crossbow rule simplified); Strength Surge (domain) rides this one swing
